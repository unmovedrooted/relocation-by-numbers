...
export function runRetirementTimeline(input: TimelineInput) {
  ...
  for (let year = input.startYear; year <= input.endYear; year++) {
    ...
    const yearInput: HouseholdYearInput = { year, distributionDate: `${year}-12-31`, filing: input.filing, state: input.state,
      stateTreatment: input.stateTreatment, cityId: input.cityId, newYorkContract: input.newYorkContract, marylandContract: input.marylandContract, indianaContract: input.indianaContract, dcContract: input.dcContract, illinoisContract: input.illinoisContract, newJerseyContract: input.newJerseyContract, pennsylvaniaContract: input.pennsylvaniaContract, coloradoContract: input.coloradoContract, newMexicoContract: input.newMexicoContract, minnesotaContract: input.minnesotaContract, utahContract: input.utahContract, connecticutContract: input.connecticutContract, vermontContract: input.vermontContract, montanaContract: input.montanaContract, rhodeIslandContract: input.rhodeIslandContract, californiaContract: input.californiaContract, virginiaContract: input.virginiaContract, arizonaContract: input.arizonaContract, georgiaContract: input.georgiaContract, northCarolinaContract: input.northCarolinaContract, southCarolinaContract: input.southCarolinaContract, ohioContract: input.ohioContract, massachusettsContract: input.massachusettsContract, iowaContract: input.iowaContract, mississippiContract: input.mississippiContract, missouriContract: input.missouriContract, washingtonContract: input.washingtonContract, alabamaContract: input.alabamaContract, arkansasContract: input.arkansasContract, delawareContract: input.delawareContract, projection: input.taxProjection, people, accounts: yearAccounts, income,
