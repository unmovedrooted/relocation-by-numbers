 ...
 * Verified 2026 parameters: TAX 601(a)(1)(B)(vii), (c)(1)(B)(vii), (d-5).
 ...
 * Future years require explicit frozen-2026 consent, NOT a current-law forecast.
 */
export function newYorkTax(input: HouseholdTaxInput, federalAgi: number, taxableBenefits: number) {
  if (input.newYorkContract !== "fixed-2026-precredit") throw new RangeError("Confirm the restricted New York planning assumptions.");
  ...
  const rates = [.039,.044,.0515,.054,.059,.0685,.0965,.103,.109];
  ...
  let recapture = 0;
  if (nyAgi > 25000000) recapture = taxable * .109 - base;
    ...
    recapture = row ? row[2] + row[3] * fraction(row[4])
      : Math.max(0, taxable * (married ? .054 : .059) - base) * fraction(107650);
  }
