...
export function estimateHouseholdTax(input: HouseholdTaxInput) {
  ...
  const localTax = ny ? ny.localTax : md ? md.localTax : inTax ? inTax.localTax : dc ? dc.localTax : il ? il.localTax : nj ? nj.localTax : pa ? pa.localTax : co ? co.localTax : nm ? nm.localTax : mn ? mn.localTax : ut ? ut.localTax : ct ? ct.localTax : vt ? vt.localTax : mt ? mt.localTax : ri ? ri.localTax : ca ? ca.localTax : va ? va.localTax : az ? az.localTax : ga ? ga.localTax : nc ? nc.localTax : sc ? sc.localTax : oh ? oh.localTax : ma ? ma.localTax : location ? 0 : null;
  const stateTax = ny ? ny.stateTax : md ? md.stateTax : inTax ? inTax.stateTax : dc ? dc.stateTax : il ? il.stateTax : nj ? nj.stateTax : pa ? pa.stateTax : co ? co.stateTax : nm ? nm.stateTax : mn ? mn.stateTax : ut ? ut.stateTax : ct ? ct.stateTax : vt ? vt.stateTax : mt ? mt.stateTax : ri ? ri.stateTax : ca ? ca.stateTax : va ? va.stateTax : az ? az.stateTax : ga ? ga.stateTax : nc ? nc.stateTax : sc ? sc.stateTax : oh ? oh.stateTax : ma ? ma.stateTax : location ? location.stateTax : estimateNetBreakdown({ grossAnnual: Math.max(0, agi), state: input.state,
