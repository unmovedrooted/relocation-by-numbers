...
export function estimateHouseholdTax(input: HouseholdTaxInput) {
  ...
  const localTax = ny ? ny.localTax : md ? md.localTax : inTax ? inTax.localTax : dc ? dc.localTax : il ? il.localTax : nj ? nj.localTax : pa ? pa.localTax : co ? co.localTax : nm ? nm.localTax : location ? 0 : null;
  const stateTax = ny ? ny.stateTax : md ? md.stateTax : inTax ? inTax.stateTax : dc ? dc.stateTax : il ? il.stateTax : nj ? nj.stateTax : pa ? pa.stateTax : co ? co.stateTax : nm ? nm.stateTax : location ? location.stateTax : estimateNetBreakdown({ grossAnnual: Math.max(0, agi), state: input.state,
  ...
  return Object.freeze({ taxYear: input.year, federalBaseYear: HOUSEHOLD_TAX_YEAR, isProjection: input.year !== 2026,
    ...
    warnings: Object.freeze([
      ny ? ny.warning : md ? md.warning : inTax ? inTax.warning : dc ? dc.warning : il ? il.warning : nj ? nj.warning : pa ? pa.warning : co ? co.warning : nm ? nm.warning : location ? location.warning : "State tax uses the existing 2025 wage-based proxy on federal AGI, not verified retirement-specific state rules; local taxes are excluded.",
