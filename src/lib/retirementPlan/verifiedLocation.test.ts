...
describe("restricted retirement locations",()=>{
  ...
  it.each(STATES.filter(state=>!["fl","tx","ny","md","in","dc","il","nj","pa","co","nm","mn","ut","ct","vt","mt","ri","ca","va","az","ga","nc"].includes(state.code)))("blocks $name without silently using a proxy",({code})=>{
