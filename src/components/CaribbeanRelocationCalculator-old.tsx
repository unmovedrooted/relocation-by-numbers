...
import {
  hasCaribbeanTaxEstimate,
  estimateCaribbeanIncomeTax,
  CARIBBEAN_TAX_ASSUMPTIONS_LABEL,
} from "@/lib/caribbeanTax";
...
// ---------------------------------------------------------------------------
// CARIBBEAN & CENTRAL AMERICA CONFIG
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// VISA CONTEXT CARD — Caribbean-specific
// ---------------------------------------------------------------------------
...
export default function CaribbeanRelocationCalculator() {
  ...
  // Default: from US NYC, to Panama City
...
useEffect(() => {
  if (selectedCityDefaults) return; // city defaults take priority
...
if (vToCountry && CARIBBEAN_COUNTRIES.includes(vToCountry as any)) {
  setToCountry(vToCountry)
 ...
 const results = useMemo(() => {
  ...
  let targetTaxEstimate: ReturnType<typeof estimateCaribbeanIncomeTax> | null = null;

  if (hasCaribbeanTaxEstimate(toCountry)) {
    targetTaxEstimate = estimateCaribbeanIncomeTax({
      countryCode: toCountry,
      grossAnnualIncome: annualIncome,
      filingStatus: filing === "married" ? "married_joint" : "single",
      convertAnnualIncomeToTaxCurrency: (income, code) => {
  const converted = convertUsdToLocal(income, code);
  console.log("tax input (local):", converted, code);
  return converted;
},
...
console.log("tax output:", targetTaxEstimate?.tax, "effectiveRate:", targetTaxEstimate?.effectiveRate);
console.log("DO rate in scope:", USD_TO_LOCAL["DO"]);
 
    
  const targetAnnualTaxUsd =
  targetTaxEstimate?.tax != null
    ? convertLocalToUsd(targetTaxEstimate.tax, toCountry)
    : 0;

const targetTaxRate =
  annualIncome > 0 ? targetAnnualTaxUsd / annualIncome : 0;
...
const currentAnnualTaxUsd = annualIncome * currentTaxRate;
  ...
  return {
    ...
    currentAnnualTaxUsd,
    targetAnnualTaxUsd,
    targetTaxLabel: targetTaxEstimate ? "Estimated annual income tax" : "Tax estimate unavailable",
    targetTaxNotes: targetTaxEstimate?.notes ?? [],
    netMonthlyFrom,
    netMonthlyTo,
    monthlyIncomeDiff,
    groceriesFrom,
    ...
    <div className="text-slate-900">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-2xl bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/60">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label className="text-sm">
                <div className={labelHeadCls}>
                  Target country
                  <InfoTip align="right" text="Filtered to Caribbean and Central American destinations. Use the International or South America calculator for other regions." />
                </div>
          ...
          {/* Visa Context — Caribbean-specific */}
...
<div>Target country: <span className="font-semibold">{getCaribbeanCountryByCode(toCountry)?.name ?? toCountry}</span></div>
  ...
  <div className="flex items-center gap-2 text-xs">
    ...
    <InfoTip text="Describes how complete the tax estimate is for this destination. Some countries are modeled more precisely than others." />
  </div>
  <div className="mt-2 space-y-1">
    {results.targetTaxNotes.map((note, i) => (
      <div key={i} className="text-sm leading-6 text-slate-700">• {note}</div>
          ...
          {/* Monthly Flexibility — teal accent */}
