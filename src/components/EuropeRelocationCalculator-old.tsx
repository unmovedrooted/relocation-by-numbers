...
// All European country codes supported in your existing data
...
// Visa / permit context per European country for non-EU citizens
...
// ---------------------------------------------------------------------------
// VISA CONTEXT CARD — Europe-specific
// ---------------------------------------------------------------------------
...
export default function EuropeRelocationCalculator() {
  ...
  // All countries (origin can be anything; destination filtered to Europe)
  ...
  // Auto-fill defaults when destination city changes
  useEffect(() => {
  ...
  const results = useMemo(() => {
    ...
    return {
      ...
      currentTaxModel: currentTaxEstimate.model, currentTaxConfidence: currentTaxEstimate.confidence,
      currentTaxLabel: currentTaxEstimate.label, currentTaxNote: currentTaxEstimate.note,
      targetTaxModel: targetTaxEstimate.model, targetTaxConfidence: targetTaxEstimate.confidence,
    ...
    <div className="text-slate-900">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        ...
        <div className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label className="text-sm">
                ...
                <select className={selectCls} value={salaryType} onChange={(e) => setSalaryType(e.target.value as SalaryType)}>
                  ...
                  <option value="local">Local salary in Europe</option>
          ...
          {/* Visa & Permit Context — Europe-specific */}
          ...
          <div className="rounded-2xl bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/60">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="text-sm">
                ...
                <input className={inputCls} type="number" value={visaCost} onChange={(e) => setVisaCost(e.target.value)} placeholder=" " />
              </label>
              <label className="text-sm">
                <div className={labelHeadCls}>One-way flight estimate</div>
                <input className={inputCls} type="number" value={flightCost} onChange={(e) => setFlightCost(e.target.value)} placeholder=" " />
              </label>
              <label className="text-sm">
                <div className={labelHeadCls}>Shipping / baggage estimate</div>
                <input className={inputCls} type="number" value={shippingCost} onChange={(e) => setShippingCost(e.target.value)} placeholder=" " />
              </label>
              <label className="text-sm">
                <div className={labelHeadCls}>Temporary housing estimate</div>
                <input className={inputCls} type="number" value={temporaryStay} onChange={(e) => setTemporaryStay(e.target.value)} placeholder=" " />
              </label>
              <label className="text-sm">
                <div className={labelHeadCls}>Setup / admin estimate</div>
                <input className={inputCls} type="number" value={adminFees} onChange={(e) => setAdminFees(e.target.value)} placeholder=" " />
              </label>
              <label className="text-sm">
                <div className={labelHeadCls}>Furniture / setup estimate</div>
                <input className={inputCls} type="number" value={furnitureSetup} onChange={(e) => setFurnitureSetup(e.target.value)} placeholder=" " />
              </label>
              <label className="text-sm sm:col-span-2">
                <div className={labelHeadCls}>Recommended cash buffer</div>
                <input className={inputCls} type="number" value={emergencyCashBuffer} onChange={(e) => setEmergencyCashBuffer(e.target.value)} placeholder=" " />
              </label>
            </div>
        ...
        <div className="space-y-3">
                ...
                <>
                  ...
                  <div className="mt-3 rounded-2xl border border-indigo-200 bg-indigo-50/80 px-4 py-3 shadow-sm">
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <InfoTip text="This describes how complete or simplified the tax estimate is for the selected European destination. Some countries are modeled more precisely than others." />
                    </div>
          ...
          {/* Monthly Flexibility — indigo accent */}
