...
export const VERIFIED_RETIREMENT_STATES: readonly string[] = ["fl", "tx", "ny", "md", "in", "dc"];
...
export function verifiedRetirementLocation(state: string, cityId = "") {
  ...
  if (!VERIFIED_RETIREMENT_STATES.includes(state)) throw new RangeError("Retirement income-tax treatment is not yet verified for this state. Only Florida, Texas, restricted New York, restricted Maryland, restricted Indiana and restricted DC scenarios are enabled; no zero-tax fallback has been used.");
  ...
  const PRE_CREDIT_NAMES: Partial<Record<StateCode, string>> = { ny: "New York", md: "Maryland", in: "Indiana", dc: "DC" };
