...
describe("restricted retirement locations",()=>{
  ...
  it.each(STATES.filter(state=>!["fl","tx","ny","md","in","dc","il","nj","pa","co","nm","mn","ut","ct","vt","mt","ri","ca","va","az","ga","nc","sc","oh","ma"].includes(state.code)))("blocks $name without silently using a proxy",({code})=>{
  ...
  it("enforces the restriction in the engine, not just in the form",()=>{
    ...
    expect(()=>runRetirementTimeline({...input,state:"wi"})).toThrow(/not yet verified/);
