...
export function runRetirementTimeline(input: TimelineInput) {
  ...
  for (let year = input.startYear; year <= input.endYear; year++) {
    ...
    const yearInput: HouseholdYearInput = { year, distributionDate: `${year}-12-31`, filing: input.filing, state: input.state,
      stateTreatment: input.stateTreatment, cityId: input.cityId, newYorkContract: input.newYorkContract, marylandContract: input.marylandContract, indianaContract: input.indianaContract, dcContract: input.dcContract, illinoisContract: input.illinoisContract, newJerseyContract: input.newJerseyContract, pennsylvaniaContract: input.pennsylvaniaContract, coloradoContract: input.coloradoContract, projection: input.taxProjection, people, accounts: yearAccounts, income,
