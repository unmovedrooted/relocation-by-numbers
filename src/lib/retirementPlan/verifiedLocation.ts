...
export const VERIFIED_RETIREMENT_STATES: readonly string[] = ["fl", "tx", "ny"];
...
export function verifiedRetirementLocation(state: string, cityId = "") {
  ...
  if (!VERIFIED_RETIREMENT_STATES.includes(state)) throw new RangeError("Retirement income-tax treatment is not yet verified for this state. Only Florida, Texas and restricted New York scenarios are enabled; no zero-tax fallback has been used.");
  ...
  return { state: state as StateCode, cityId, stateTax: 0, localTax: 0,
    warning: state === "ny" ? "New York requires the separate pre-credit state/local tax calculation; this location validation is not a tax estimate." : `${state === "tx" ? "Texas" : "Florida"} full-year resident individual income-tax treatment: state and local income tax are zero. Assumes all modeled income is free of another jurisdiction's income tax; cross-border work and part-year moves are unsupported. Holds current treatment through the horizon. Property, sales and business taxes are not calculated.` };
}
